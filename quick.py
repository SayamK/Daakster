...
def main():
    ...
    for i in msgs:
      ...
      orig.append(txt)
      sub = (GetMessage(build_service(get_credentials()),"me",i.get('id')).get('payload').get('headers')[16])
      name = (GetMessage(build_service(get_credentials()),"me",i.get('id')).get('payload').get('headers')[14])
      if sub.get('name') == "Subject":
        subjects.append(sub.get('value'))
      if name.get('name') == "From":
        namee.append(name.get('value'))
      try:
    ...
    for i in namee:
      for j in i:
        if j==' ':
          b=1
        elif b==1 and str.isalpha(str(j)):
          ans.append(j)
          b=0
        else:
          b=0
...
@app.route("/pro")
def home():
  return render_template("inbox.html", results=(results), subjects=subnew, name=ans)
@app.route("/mail")
def mail():
  return render_template("mails.html", results=(results), subjects=subnew, name=ans)
